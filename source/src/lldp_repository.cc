...
namespace ndisc
{
    std::expected<LldpRepository, int> LldpRepository::Create()
    {
        ...
        return LldpRepository(std::move(eth_broadcast_fd));
    }
    ...
    void LldpRepository::CreateLldpStateMachine(unsigned int idx)
    {
        if (current_state_.contains(idx))
        {
            LldpSender &lldp_sender = current_state_.at(idx);
            lldp_sender.NewNeighbour();
        }
    }

    void LldpRepository::UpdateState(const std::map<unsigned int, DeviceData> &new_state)
    {
        for (const auto &[index, new_device_state] : new_state)
        {
            if (current_state_.contains(index))
            {
                LldpSender &lldp_sender = current_state_.at(index);
                lldp_sender.Update(new_device_state);
            }
            else
            {
                current_state_.emplace(index, LldpSender(ethernet_broadcast_socket_, new_device_state));
    ...
    {
        if (repository.AreReadersIdle())
        {
            lldp.UpdateState(repository.GetDevices());
        }
    }
} // namespace ndisc